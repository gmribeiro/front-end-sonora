...
import api from '../../api'; // Certifique-se de que este 'api' é sua instância Axios configurada
...
function MeuPerfil() {
  ...
  // Considerar substituir alert por um modal customizado para melhor UX
  const showError = (msg) => alert(msg);
  const showSuccess = (msg) => alert(msg);

  // --- FUNÇÃO fetchProfileImage ATUALIZADA ---
  const fetchProfileImage = useCallback(async (token) => {
    try {
      // Faz a requisição. Axios, em ambiente de navegador, automaticamente
      // segue redirecionamentos 302. Não precisamos de 'responseType: blob' aqui,
      // pois queremos a URL final, não o conteúdo binário diretamente da primeira requisição.
      const response = await api.get('/auth/user/me/profile-image', {
        headers: { Authorization: `Bearer ${token}` },
      });

      // Após o redirecionamento (se houver), response.request.responseURL conterá a URL final
      // para onde a requisição foi redirecionada (a URL da imagem no S3/Google).
      if (response.request && response.request.responseURL) {
        setProfileImageUrl(response.request.responseURL);
        console.log("DEBUG: Imagem de perfil carregada de (URL final):", response.request.responseURL);
      } else {
        // Cenário de fallback se por algum motivo a URL final não for encontrada
        console.warn("DEBUG: Nenhuma URL de imagem final encontrada na resposta.");
        setProfileImageUrl(null);
      }
    } catch (error) {
      // O backend retorna status 404 se a imagem de perfil não for encontrada.
      if (error.response && error.response.status === 404) {
        console.warn('DEBUG: Imagem de perfil não encontrada (404). Definindo como null.');
        setProfileImageUrl(null);
      } else {
  ...
  // --- FIM DA FUNÇÃO fetchProfileImage ATUALIZADA ---
  ...
  useEffect(() => {
    ...
    fetchUser();
  }, [navigate, fetchProfileImage]); // Dependência fetchProfileImage adicionada para useCallback
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      fetchProfileImage(token); // Recarrega a imagem após o upload
          ...
          <div className="flex flex-col gap-8 lg:gap-10 max-w-6xl mx-auto">
            <div className="bg-white rounded-lg shadow-2xl p-6 transform transition-transform duration-300 hover:scale-[1.005] border border-gray-100 backdrop-blur-sm bg-opacity-90">
              ...
              <div className="flex flex-col items-center gap-5">
                {/* Renderiza a imagem de perfil usando a URL diretamente */}
                {profileImageUrl ? (
                    <img
                        src={profileImageUrl}
                        alt="Perfil"
                ...
                <button
                    ...
                    className="!bg-[#342e5a] hover:!bg-[#564A72] text-white px-7 py-3.5 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-[#342e5a] focus:ring-opacity-75 font-semibold text-base"
                >
                  Editar Perfil
                </button>
                <button
                    onClick={() => setShowSenhaModal(true)}
                    className="!bg-[#c2a0bb] hover:!bg-[#A48BB3] text-white px-7 py-3.5 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-[#c2a0bb] focus:ring-opacity-75 font-semibold text-base mt-2"
                >
                  Alterar Senha
                </button>
              </div>
