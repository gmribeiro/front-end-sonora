...
function MeusContratos() {
    ...
    useEffect(() => {
        const carregarDadosIniciais = async () => {
            ...
            try {
                ...
                const musicoLogado = musicosResponse.data.find(musico => musico.usuario && musico.usuario.id === userResponse.data.id);
    ...
    const handleAceitarContrato = async (idEvento, idMusicoQueEstaAceitando, nomeEvento, idGeneroMusical) => {
        const confirmacao = window.confirm("Você tem certeza que deseja aceitar este contrato e ser escalado para o evento?");
        if (!confirmacao) {
            return;
        }

        const currentContractId = `${idEvento}-${idMusicoQueEstaAceitando}`;
        ...
        try {
            ...
            try {
                ...
                const escalaResponse = await axios.get(`/escalas/event/${idEvento}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                if (axios.isAxiosError(getError) && getError.response && getError.response.status === 404) {
            ...
            const escalaData = {
                ...
                idsMusicos: Array.from(idsMusicosParaEnviar) // Converte o Set de volta para Array
            };
            // --- FIM DA ALTERAÇÃO ---
            ...
            await axios.put(`/contratos/${idEvento}/${idMusicoQueEstaAceitando}/activate`, {}, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            setMeusContratosArtista(prevContratos =>
                prevContratos.map(contrato =>
                    (contrato.idContrato.evento.idEvento === idEvento &&
                        contrato.idContrato.musico.idMusico === idMusicoQueEstaAceitando)
            ...
            console.error('Erro ao aceitar contrato ou criar/atualizar escala:', error);
